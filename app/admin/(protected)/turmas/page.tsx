  ...
  return (
    <div className="container mx-auto px-6 max-w-[1200px] py-10">
      <div className="mb-7">
        ...
        <p className="text-muted-2 m-0">Cadastre novas turmas ou disciplinas para ampliar a base de dados. Em modo demo, os formulários apenas validam o payload (sem persistir).</p>
      </div>
          ...
          return (
            <div key={t.id} className="bg-bg-alt border border-line rounded-[14px] p-5 shadow-sm">
              <header className="mb-3">
                ...
                <small className="text-muted-2 text-xs">{t.ano ? `Ano: ${t.ano}` : "Ano não informado"} · {tgrupos.length} disciplinas</small>
                <p className="text-muted-2 text-sm mt-2 m-0">{t.descricao}</p>
              </header>
                ...
                {tgrupos.map((g) => (
                  <li key={g.id} className="flex items-center justify-between bg-bg rounded-md px-3 py-2 text-sm">
                    <span className="flex items-center gap-2">
                      ...
                      <span className="text-muted-2 text-xs">({g.notas.length} alunos)</span>
                    </span>
